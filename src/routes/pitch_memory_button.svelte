<script lang="ts">
    let { 
        perc = $bindable(),
        show_notes_memory_traces = $bindable()
    } = $props();


    let show_pitch_memory_button_is_hovered = $state(false);
</script>
...
onmouseenter={() => show_pitch_memory_button_is_hovered = true} 
onmouseleave={() => show_pitch_memory_button_is_hovered = false}>
    <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" viewBox="0 0 512 512" 
    class="{show_notes_memory_traces ? 'active_button' : ''}" style="cursor: pointer;"
    onclick={() => {
        show_notes_memory_traces = !show_notes_memory_traces;
    }}>
    ...
    {#if show_pitch_memory_button_is_hovered}
        <div id="new_keys_settings_dropdown">
            <h style="color: var(--very-dark); font-size: 16px;">
                Show notes' pitch memory traces
            </h>
            <div>
                <h style="color: var(--very-dark);">HE weakness (%):</h>
                <input 
